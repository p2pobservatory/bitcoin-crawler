...
func check_peers_ltcd(mode string) {
	var activePeerFiles []string
	var respondedPeerListDir string

	if mode == "active" {
		activePeerFiles = getActivePeerFiles(currency, ipversion)
		respondedPeerListDir = datadir+"bitcoin-crawler/responded_peerLists/"
	} else {
		activePeerFiles = getAllPeerFiles(currency, ipversion)
		respondedPeerListDir = datadir+"bitcoin-crawler/responded_peerLists_all/"
	}
	...
	for _, activePeerFile := range activePeerFiles {
		...
		for addr := range activePeerAddrs {
			...
			go func(addr string) {
				...
				responds := sendVersionMessage_ltcd(addr)
...
func sendVersionMessage_ltcd(addr string) bool {
