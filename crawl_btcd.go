...
func versionMsg_btcd(conn net.Conn, host string, port int) wire.Message {
	var localIP string
...
// PeerEntry holds the full information from a single addr message entry.
type PeerEntry struct {
	Address   string `json:"address"`
	Services  uint64 `json:"services"`  // capabilities bitmask (e.g. NODE_NETWORK=1, NODE_BLOOM=4, NODE_WITNESS=8)
	Timestamp string `json:"timestamp"` // nTime: when the peer was last seen
}

func savePeerTable_btcd(msg *wire.MsgAddr, host string, port int) {
...
func getPeers_btcd(addr string, peers map[string]bool, activePeers map[string]bool, verbosity int, mu *sync.Mutex) error {
	...
	for i := 0; i < maxRetry; i++ {
		...
		resp := versionMsg_btcd(conn, host, port)
		print(resp)
		switch resp.(type) {
		case *wire.MsgVersion:
			...
			case *wire.MsgVerAck:
				...
				case *wire.MsgAddr:
					...
					mu.Unlock()
					savePeerTable_btcd(msg, host, port)
				default:
					if verbosity >= 3 {
...
func getPeersBatch_btcd(addrBatch []string, peers map[string]bool, activePeers map[string]bool, verbosity int, mu *sync.Mutex, concurrencyControl chan struct{}) error {
	...
	for _, addr := range addrBatch {
		...
		go func(addr string) {
			...
			defer wg.Done()
			err := getPeers_btcd(addr, peers, activePeers, verbosity, mu)
			if err != nil {
				// handle error
				fmt.Println(err)
			}
	...
	for {
		...
		for i := 0; i < len(vars.IPAddrs); i += batchSize {
			...
			go func(addrBatch []string) {
				...
				err := getPeersBatch_btcd(addrBatch, peers, active, verbosity, vars.Mu, vars.ConcurrencyControl)
				if err != nil {
					if verbosity >= 3 {
